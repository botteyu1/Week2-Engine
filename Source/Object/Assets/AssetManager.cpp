...
void UAssetManager::ObjParsing(const FString& filePath, TArray<FVertexTextureArray>& outVertex, TArray<uint32>& outIndex) 
{
	std::ifstream objFile(filePath.ToWideString());
	...
	if (objFile)
	{
		...
		TMap<VertexKey, int> vertexMap; // Key: (vertexIdx * 1000000 + uvIdx * 1000 + normalIdx), Value: New Vertex Index
		TArray<FVertexTextureArray> newVertices;
		TArray<uint32> newIndices;
		...
		for (const auto& face : faceData) {
			...
			if (!vertexMap.Contains(vertexKey)) {
				...
				FVertexTextureArray simpleVertex(pos.X - 0.5f, pos.Y - 0.5f, pos.Z - 0.5f, 1.0f, 0.0f, 0.0f, 1.0f, u, v, norm.X, norm.Y, norm.Z, 0);
				newVertices.Add(simpleVertex);
				vertexMap.Add(vertexKey, newIndex);
			}
